...
export function TodayHealth() {
  const health = useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ count: priceRows, error: priceError }, { count: coveredSymbols, error: coverageError }, { data: signals, error: signalError }, { data: jobs, error: jobError }] = await Promise.all([
      ...
      if (priceError || coverageError || signalError || jobError) throw priceError || coverageError || signalError || jobError
      const job = jobs?.[0]
      ...
      return {
        priceRows: priceRows ?? 0, coveredSymbols: coveredSymbols ?? 0, newestDate,
  ...
  return <article className="panel discipline-card today-health-card">
    ...
    {health.isLoading ? <p className="muted">Đang kiểm tra độ phủ dữ liệu…</p> : health.isError ? <p className="negative">Không tải được trạng thái dữ liệu.</p> : <>
      <div className="health-metrics"><span><Database size={15}/><b>{data?.coveredSymbols ?? 0}/205</b><small>Mã có giá</small></span><span><b>{(data?.priceRows ?? 0).toLocaleString('vi-VN')}</b><small>Bản ghi giá</small></span><span><b>{data?.coreActions ?? 0}</b><small>Action mới</small></span><span><b>{data?.coreWatch ?? 0}</b><small>Core WATCH</small></span></div>
