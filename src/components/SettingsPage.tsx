...
export function SettingsPage({ authenticated, isAdmin = true }: { authenticated: boolean; isAdmin?: boolean }) {
  ...
  return <section className="workspace-page settings-page">
    ...
    <div className="timeframe-tabs" role="tablist" aria-label="Cài đặt">{!isAdmin&&<button className="active">Giới thiệu</button>}{isAdmin&&<><button className={tab === 'guide' ? 'active' : ''} onClick={() => setTab('guide')}>Hướng dẫn</button><button className={tab === 'account' ? 'active' : ''} onClick={() => setTab('account')}>Tài khoản</button></>}</div>
    {!isAdmin || tab === 'intro' ? <Intro/> : tab === 'account' ? (
    ...
    ) : <Guide authenticated={authenticated} rules={rules.data ?? []} loading={rules.isLoading} includeIntro={isAdmin}/>}
  </section>
}
...
function Guide({ authenticated, rules, loading, includeIntro }: { authenticated: boolean; rules: any[]; loading: boolean; includeIntro: boolean }) {
  ...
  return <div className="guide-list">{includeIntro&&<Intro/>}<a className="guide-universe-card" href="#universe"><span>Danh mục</span><strong>Danh sách cổ phiếu</strong><small>Xem đầy đủ mã đang hoạt động và tải danh sách.</small></a><EngineGuide/>
