...
const actionLabel:Record<string,string>={EXIT:'Thoát vị thế',REDUCE:'Giảm tỷ trọng',ADD:'Mua thêm',PROBE_BUY:'Mua thăm dò'}
...
export function SignalDecisionBoard({onSelect}:{onSelect:(symbol:string)=>void}) {
  ...
  const context=rows.filter(row=>row.action==='WATCH'&&(row.state!=='WATCH_SETUP'||row.trigger==null||row.stop==null||row.expiry==null)).sort((a,b)=>b.score-a.score).slice(0,6)
  const Row=({row,kind}:{row:Row;kind:'action'|'watch'|'context'})=><a href="#analysis" className={`decision-row ${kind}`} onClick={()=>onSelect(row.symbol)}><div><strong>{row.symbol}</strong><small>{row.sector??'Chưa phân ngành'} · {row.timeframe}</small></div>{kind==='action'?<span className={`action-pill ${row.action.toLowerCase()}`}>{actionLabel[row.action]??row.action}</span>:kind==='watch'?<span className="decision-plan">Trigger {formatMarketPrice(row.trigger)}<small>Hết hạn {row.expiry?formatDate(row.expiry):'—'}</small></span>:<span className="decision-context">{stateLabel[row.state]??'Bối cảnh'}<small>{row.reasons[0]??'Theo dõi'}</small></span>}</a>
  const Empty=({children}:{children:string})=><p className="decision-empty">{children}</p>
  return <section className="signal-decision-board" aria-label="Ba vùng quyết định tín hiệu"><article className="panel decision-zone actionable-zone"><header><div><span>01 · Hành động</span><h2>Cần hành động</h2><small>Chỉ có quyết định giao dịch rõ ràng cho phiên này.</small></div><a href="#screener">Xem tất cả</a></header>{query.isLoading?<Empty>Đang tải tín hiệu…</Empty>:actionable.length?actionable.map(row=><Row key={row.id} row={row} kind="action"/>):<Empty>Chưa có hành động mới trong phiên.</Empty>}</article><article className="panel decision-zone watch-zone"><header><div><span>02 · Chờ xác nhận</span><h2>Chờ trigger</h2><small>Setup có sẵn giá kích hoạt, stop và hạn theo dõi.</small></div><a href="#screener">Lọc setup</a></header>{watch.length?watch.map(row=><Row key={row.id} row={row} kind="watch"/>):<Empty>Chưa có setup gần trigger.</Empty>}</article><article className="panel decision-zone context-zone"><header><div><span>03 · Bối cảnh</span><h2>Bối cảnh</h2><small>Xu hướng, rủi ro và Prot Flow — không phải khuyến nghị mua.</small></div><a href="#analysis">Prot Flow</a></header>{context.length?context.map(row=><Row key={row.id} row={row} kind="context"/>):<Empty>Không có bối cảnh nổi bật trong phiên.</Empty>}</article></section>
}
