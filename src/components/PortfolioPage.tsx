...
export function PortfolioPage({ authenticated }: { authenticated: boolean }) {
  ...
  async function submitTransaction(event: FormEvent) {
    ...
    const payload = (nextAction: TransactionAction) => ({ p_action: nextAction, p_quantity: nextAction === 'STOP_UPDATE' ? 0 : quantity, p_price: nextAction === 'STOP_UPDATE' ? null : price || null, p_stop_price: stop || null, p_note: note || null, p_trading_date: tradingDate })
