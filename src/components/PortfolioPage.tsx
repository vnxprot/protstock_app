...
export function PortfolioPage({ authenticated }: { authenticated: boolean }) {
  ...
  async function submitTransaction(event: FormEvent) { event.preventDefault(); const symbolId = symbols.data?.find(item => item.symbol === symbol)?.id; if (!supabase || !portfolio.data || !symbolId) return notify('Hãy chọn mã từ gợi ý'); const payload = { p_action: action, p_quantity: action === 'STOP_UPDATE' ? 0 : quantity, p_price: action === 'STOP_UPDATE' ? null : price || null, p_stop_price: stop || null, p_note: note || null, p_trading_date: tradingDate }; const { error } = editingTransaction ? await supabase.rpc('update_portfolio_transaction', { p_transaction_id: editingTransaction.id, ...payload }) : await supabase.rpc('record_portfolio_transaction', { p_portfolio_id: portfolio.data.id, p_symbol_id: symbolId, ...payload }); if (error) return notify(`Không thể lưu giao dịch: ${error.message}`); client.invalidateQueries({ queryKey: ['portfolio'] }); client.invalidateQueries({ queryKey: ['portfolio-transactions'] }); client.invalidateQueries({ queryKey: ['portfolio-analytics-transactions'] }); client.invalidateQueries({ queryKey: ['journal'] }); setTradeOpen(false); notify(editingTransaction ? 'Đã cập nhật giao dịch và vị thế' : `${transactionLabels[action]} đã lưu`) }
  ...
  return <section className="workspace-page"><div className="page-title-row"><div><h1>Danh mục</h1><p className="muted">Giá: VND/cổ phiếu · Khối lượng: cổ phiếu · Dữ liệu giá gần nhất: {latestDate ? latestDate.split('-').reverse().join('/') : '—'}.</p></div><div className="page-actions"><div className="report-menu"><button className="secondary-button" onClick={() => setReportOpen(value => !value)}><Download size={16}/> Báo cáo tổng quan <ChevronDown size={14}/></button>{reportOpen && <div className="report-menu-popover"><button onClick={() => void exportReport('excel')}><FileSpreadsheet size={16}/> Tải Excel <small>.xlsx · số liệu đầy đủ</small></button><button onClick={() => void exportReport('pdf')}><FileText size={16}/> Tải PDF <small>.pdf · bản tóm tắt</small></button></div>}</div>{portfolio.data && <button className="secondary-button" onClick={openCapitalSetup}>Điều chỉnh vốn</button>}<button className="primary-button" onClick={() => portfolio.data ? openTrade() : openCapitalSetup()}><Plus size={16}/>{portfolio.data ? 'Giao dịch' : 'Khởi tạo danh mục'}</button></div></div>
