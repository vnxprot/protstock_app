...
function App({ authenticated = false, profile = null }: { authenticated?: boolean; profile?: UserProfile | null }) {
  ...
  return <div className={collapsed ? 'app-shell sidebar-collapsed' : 'app-shell'}>
    ...
    {moreOpen && <div className="sheet-backdrop" onMouseDown={() => setMoreOpen(false)}><section className="bottom-sheet" onMouseDown={e => e.stopPropagation()}><div className="sheet-handle"/><div className="sheet-title"><div><h2>Mở thêm công cụ</h2></div><button className="icon-button" onClick={() => setMoreOpen(false)}><X size={20}/></button></div><div className="sheet-grid">{(isAdmin?[...moreMobile,{id:'admin',icon:ShieldCheck,label:'Quản trị'}]:clientModules.filter(item=>item.id==='settings')).map(item => <a href={`#${item.id}`} key={item.id}><span><item.icon size={21}/></span><strong>{item.label}</strong><small>{item.id === 'admin'?'Tài khoản và phiên':item.id === 'settings'?'Giới thiệu và hệ thống':'Công cụ'}</small></a>)}</div></section></div>}
...
function MarketContextCard() {
  ...
  const sectorLeaders = Array.isArray(data?.breadth?.sector_breadth) ? data.breadth.sector_breadth.filter((item:any) => item?.sample_size >= 2).sort((a:any,b:any) => Number(b.market_health_score ?? -1) - Number(a.market_health_score ?? -1)).slice(0, 4) : []
