...
function ExecutiveKpiStrip({ signalCount, highConfluenceCount, favoritesCount, activeSignalCount, universeCount, connectionLabel, health }: { signalCount: number; highConfluenceCount: number; favoritesCount: number; activeSignalCount: number; universeCount: number; connectionLabel: string; health?: { latest_price_date: string | null; failed_jobs_7d: number } }) {
  return <section className="executive-kpi-strip" aria-label="Tóm tắt điều hành sau phiên">
    ...
    <article className="executive-kpi-card"><span>PIPELINE</span><strong className={connectionLabel.includes('trực tuyến') ? 'positive' : 'negative'}>{connectionLabel.includes('trực tuyến') ? 'ONLINE' : 'CHECK'}</strong><small>{universeCount} mã · {health?.failed_jobs_7d ?? 0} lỗi / 7 ngày</small><small>Dữ liệu: {formatDate(health?.latest_price_date)}</small></article>
