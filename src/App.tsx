...
function App({ authenticated = false, profile = null }: { authenticated?: boolean; profile?: UserProfile | null }) {
  ...
  return <div className={collapsed ? 'app-shell sidebar-collapsed' : 'app-shell'}>
    ...
    <nav className={`bottom-nav${mobileNavCompact ? ' is-compact' : ''}`} aria-label="Điều hướng di động">{(isAdmin?primaryMobile:clientModules.filter(item=>['today','analysis','screener'].includes(item.id))).map(item => <a className={activePage === item.id ? 'active' : ''} href={`#${item.id}`} aria-label={item.label} key={item.id}><item.icon size={21}/><span>{item.id === 'analysis' ? 'Phân tích' : item.id === 'screener' ? 'Tín hiệu' : item.label}</span></a>)}<button className={moreOpen || (isAdmin?moreMobile:clientModules).some(item => item.id === activePage) ? 'active' : ''} onClick={() => setMoreOpen(true)} aria-label="Thêm công cụ"><Menu size={21}/><span>Thêm</span></button></nav>
