...
function App({ authenticated = false }: { authenticated?: boolean }) {
  ...
  const navCounts = useQuery({ queryKey: ['nav-counts'], enabled: authenticated && Boolean(supabase), queryFn: async () => { const [signals, rules] = await Promise.all([supabase!.from('signals').select('as_of_date').order('as_of_date', { ascending: false }).limit(202), supabase!.from('rules').select('id', { count: 'exact', head: true }).eq('status', 'ACTIVE')]); if (signals.error || rules.error) throw signals.error || rules.error; const latest = signals.data?.[0]?.as_of_date; return { signals: latest ? signals.data!.filter(item => item.as_of_date === latest).length : 0, rules: rules.count ?? 0 } } })
