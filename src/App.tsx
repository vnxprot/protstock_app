...
function ExecutiveKpiStrip({ signalCount, highConfluenceCount, favoritesCount, activeSignalCount, connectionLabel, health }: { signalCount: number; highConfluenceCount: number; favoritesCount: number; activeSignalCount: number; connectionLabel: string; health?: { latest_price_date: string | null } }) {
  const market = useQuery({ queryKey: ['executive-vnindex'], enabled: Boolean(supabase), staleTime: 60_000, queryFn: async () => {
    const { data: index, error } = await supabase!.from('market_indices').select('id').eq('code', 'VNINDEX').single()
    if (error) throw error
    const { data: prices, error: pricesError } = await supabase!.from('market_index_prices').select('trading_date,close').eq('index_id', index.id).order('trading_date', { ascending: false }).limit(2)
    if (pricesError) throw pricesError
    const latest = prices?.[0]; const prior = prices?.[1]
    return { latest, change: latest && prior ? ((Number(latest.close) - Number(prior.close)) / Number(prior.close)) * 100 : null }
  } })
  const trend = market.data?.change == null ? 'NEUTRAL' : market.data.change >= 0 ? 'UPTREND' : 'DOWNTREND'
  const latestIndex = market.data?.latest?.close == null ? '—' : Number(market.data.latest.close).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
  return <section className="executive-kpi-strip" aria-label="Tóm tắt điều hành sau phiên">
    <article className="executive-kpi-card"><span>VNINDEX</span><strong className="kpi-value">{latestIndex}</strong><small className={trend === 'DOWNTREND' ? 'negative' : 'positive'}>{market.data?.change == null ? 'Đang đồng bộ' : `${market.data.change >= 0 ? '+' : ''}${market.data.change.toFixed(2)}%`} · {trend}</small></article>
    <article className="executive-kpi-card"><span>TÍN HIỆU MỚI</span><strong className="kpi-value">{signalCount}</strong><small>{highConfluenceCount} đồng thuận cao</small></article>
    <article className="executive-kpi-card"><span>PIPELINE</span><strong className={connectionLabel.includes('trực tuyến') ? 'positive' : 'negative'}>{connectionLabel.includes('trực tuyến') ? 'ONLINE' : 'CHECK'}</strong><small>Dữ liệu: {formatDate(health?.latest_price_date)}</small></article>
    <article className="executive-kpi-card"><span>WATCHLIST</span><strong className="kpi-value">{favoritesCount}</strong><small>{activeSignalCount} mã có tín hiệu</small></article>
  </section>
}
...
function Dashboard({ universeCount, connectionLabel, health }: { universeCount: number; connectionLabel: string; health?: { latest_price_date: string | null; failed_jobs_7d: number } }) {
  const signals = useQuery({ queryKey: ['today-consolidated-signals'], enabled: Boolean(supabase), queryFn: async () => { const { data, error } = await supabase!.from('consolidated_signals').select('id,composite_action,confluence_score,confluence_count,as_of_date,reasons,symbols(symbol)').order('as_of_date', { ascending: false }).order('confluence_score', { ascending: false }).limit(6); if (error) throw error; return (data ?? []).map((item:any) => ({ ...item, action: item.composite_action, score: item.confluence_score })) } })
  ...
  const highConfluence=feed.filter((item:any)=>Number(item.confluence_count)>=2).length
  const watchlistSignals=feed.filter((item:any)=>favorites.includes(item.symbols?.symbol)).length
  const openChart=(symbol:string)=>{localStorage.setItem('protstock-symbol',symbol);location.hash='analysis'}
  return <section className="dashboard-page"><header className="dashboard-header"><div><span className="eyebrow">EOD INTELLIGENCE</span><h1>Tổng quan</h1><p>Không gian phân tích và quản trị giao dịch cá nhân.</p></div><a href="#screener" className="primary-button"><TrendingUp size={17}/> Mở Screener</a></header><ExecutiveKpiStrip signalCount={feed.length} highConfluenceCount={highConfluence} favoritesCount={favorites.length} activeSignalCount={watchlistSignals} connectionLabel={connectionLabel} health={health}/><section className="market-strip"><div><span className="live-dot"/><strong>{connectionLabel}</strong></div><span>Universe <b>{universeCount}</b></span><span>Dữ liệu <b>{health?.latest_price_date??'Chưa có'}</b></span><span>Lỗi 7 ngày <b className={health?.failed_jobs_7d?'negative':''}>{health?.failed_jobs_7d??0}</b></span></section><section className="dashboard-grid"><div className="dashboard-main-column"><MarketContextCard/><article className="panel signal-card"><div className="panel-title"><div><span className="eyebrow">TÍN HIỆU SAU PHIÊN</span><h2>Tín hiệu mới</h2></div><a href="#screener">Xem tất cả</a></div><div className="signal-stack">{signals.isLoading?<p className="muted">Đang tải tín hiệu…</p>:feed.length?feed.map((item:any)=><article className="dashboard-signal" key={item.id}><span className="symbol-avatar">{item.symbols?.symbol?.slice(0,2)}</span><span className="dashboard-signal-symbol"><strong>{item.symbols?.symbol}</strong><small>{item.symbols?.symbol}</small></span><span className={`action-pill ${item.action?.toLowerCase()}`}>{item.action?.replace('_',' ')}</span><span className="confluence-badge">{Number(item.confluence_count)>=2?'HIGH CONFLUENCE':'STANDARD'} · {item.confluence_count} engine</span><b>{item.score}/100</b><em>{item.as_of_date}</em><button type="button" onClick={()=>openChart(item.symbols?.symbol)} aria-label={`Xem chart ${item.symbols?.symbol}`}>Xem Chart →</button></article>):<p className="muted">Chưa có tín hiệu sau phiên.</p>}</div></article></div><aside className="dashboard-side-column"><article className="panel watch-card"><div className="panel-title"><div><span className="eyebrow">WATCHLIST</span><h2>Đang theo dõi</h2></div><button className="text-button" onClick={()=>openCommandPalette('symbols')}>+ Thêm mã</button></div>{favorites.length?favorites.map(symbol=><a href="#analysis" className="watch-row" key={symbol} onClick={()=>localStorage.setItem('protstock-symbol',symbol)}><strong>{symbol}</strong><span>Đang theo dõi</span><b>→</b></a>):<p className="muted">Chưa có mã nào trong watchlist.</p>}</article><TodayHealth/></aside></section></section>
}
export default App
