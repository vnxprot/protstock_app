...
export function canonicalEngineName(name: string) {
  return name === 'Prot Core Pack · Phân kỳ RSI + xác nhận MACD'
    ? 'Prot Core Pack · RSI MACD Divergence'
    : name
}
export function compareEngines(a: {name: string}, b: {name: string}) {
  const rank = (name: string) => { const index = engineOrder.indexOf(canonicalEngineName(name)); return index < 0 ? 999 : index }
  return rank(a.name) - rank(b.name) || a.name.localeCompare(b.name, 'vi')
}
export const engineGuides = [
  {name: 'Prot Core Engine v0.0', detail: 'Bảy nhóm: nền phẳng, cờ tăng/pennant, hai đáy, hai đỉnh, vai đầu vai/ngược, cốc tay cầm và đáy tròn. Cốc ưu tiên khung Tuần; breakout phải vượt miệng cốc với volume. Chọn một cấu trúc mạnh nhất để không cộng nhiều phiếu cho cùng biểu đồ; tín hiệu mua vẫn qua bộ lọc chung.'},
  ...
  {name: 'Prot Core Pack · T+ Pullback', detail: 'Setup ngắn hạn: hồi 3–7 phiên về EMA/SMA khi xu hướng Tuần tăng, sau đó có nến ngày hồi và volume xác nhận. Entry chỉ hợp lệ tối đa 3 phiên; time-stop 8 phiên. Không phải chiến lược bắt đáy trong downtrend và vẫn chịu market, thanh khoản, stop, danh mục.'},
]
