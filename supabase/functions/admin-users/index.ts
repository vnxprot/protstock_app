...
Deno.serve(async request => {
  ...
  if (action === 'create') {
    ...
    await admin.from('user_profiles').insert({ user_id: data.user.id, username, full_name: fullName, role: 'CLIENT', status: 'ACTIVE' })
    return Response.json({ ok: true }, { headers: cors })
  }
  if (action === 'status') { await admin.from('user_profiles').update({ status: body.status }).eq('user_id', body.user_id); if (body.status !== 'ACTIVE') await admin.auth.admin.signOut(body.user_id, 'global'); return Response.json({ ok: true }, { headers: cors }) }
  if (action === 'signout_all') { await admin.auth.admin.signOut(body.user_id, 'global'); await admin.from('user_active_sessions').delete().eq('user_id', body.user_id); return Response.json({ ok: true }, { headers: cors }) }
