...
def analyze_bars(bars: Sequence[dict], position: dict | None = None, weekly_patterns: list[dict] | None = None, monthly_snapshot: dict | None = None, benchmark_rows: Sequence[dict] | None = None, market_context: dict | None = None) -> dict:
    if not bars:
    ...
    signal, reasons = resolve_signal(pattern_dicts, snapshot_dict, position, market_context=market_context, multi_timeframe_context=multi_timeframe_context)
...
def resolve_signal(patterns: list[dict], snapshot: dict, position: dict | None = None, *, market_context: dict | None = None, multi_timeframe_context: dict | None = None, zones: Sequence[dict] | None = None) -> tuple[str, list[str]]:
    ...
    if top and top["quality_score"] >= threshold and snapshot.get("trend_state") in ("UP", "SIDEWAYS") and (snapshot.get("volume_ratio20") or 0) >= 1.3 and (snapshot.get("rsi14") or 100) < 75 and liquid:
        ...
        return _apply_long_gates(action, reasons, multi_timeframe_context, market_context)
    ready = [p for p in patterns if p["state"] == "READY"]
    return "WATCH", [f"NEAR_TRIGGER_{p['pattern_type']}" for p in ready[:2]]


def _apply_long_gates(action: str, reasons: list[str], multi_timeframe_context: dict | None, market_context: dict | None) -> tuple[str, list[str]]:
