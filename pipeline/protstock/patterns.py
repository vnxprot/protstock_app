...
def _quality(bars: Sequence[dict], start: int, support: float, resistance: float, confirmed: bool, near_breakout: bool, candle_confirmation: bool = False) -> tuple[float, dict]:
    ...
    components = {
        ...
        "candlestick_confirmation_score": 3.0 if confirmed and candle_confirmation else 0.0,
    }
    evidence = {
    ...
    return round(min(100.0, sum(components.values())), 1), evidence


def _candle_confirmation(bars: Sequence[dict], direction: str) -> bool:
    if len(bars) < 2:
        return False
    last = bars[-1]
    return (direction == "BULLISH" and (is_bullish_engulfing(bars[-2], last) or is_pin_bar(last, "BULLISH"))) or (direction == "BEARISH" and is_pin_bar(last, "BEARISH"))
...
def detect_accumulation_base(bars: Sequence[dict]) -> PatternCandidate | None:
    ...
    if depth > 0.35 or recent_range > prior_range * 1.15:
        return None
    candle_ok = confirmed and _candle_confirmation(window, "BULLISH")
    score, quality = _quality(window, 0, support, resistance, confirmed, near_breakout, candle_ok)
    return PatternCandidate(
        ...
        tuple(filter(None, ("RANGE_CONTRACTION", "VOLUME_DRY_UP" if dry_up else "", "BREAKOUT_VOLUME" if breakout_volume_ok else "", "CANDLESTICK_CONFIRMATION" if candle_ok else "", "NEEDS_VOLUME_CONFIRMATION" if closes[-1] > resistance and not breakout_volume_ok else "", "NEAR_BREAKOUT" if near_breakout else ""))),
...
def detect_double(bars: Sequence[dict], kind: str) -> PatternCandidate | None:
    ...
    score, quality = _quality(bars, first, invalidation if kind == "bottom" else neckline, neckline if kind == "bottom" else invalidation, confirmed, not confirmed, candle_ok)
    return PatternCandidate(
        pattern_type, "CONFIRMED" if confirmed else "READY", direction, first, len(bars) - 1,
        neckline, invalidation, score,
        tuple(filter(None, ("TWO_CONFIRMED_PIVOTS", "NECKLINE_BREAK" if confirmed else "NEAR_NECKLINE", "BREAKOUT_VOLUME" if volume_ok else "", "CANDLESTICK_CONFIRMATION" if candle_ok else "", "NEEDS_VOLUME_CONFIRMATION" if ((close > neckline) if kind == "bottom" else (close < neckline)) and not volume_ok else ""))),
